...
resource "null_resource" "remote_install" {
  ...
  provisioner "remote-exec" {
    inline = [
      "api_token=${random_id.token.0.hex} secret_token=${random_id.token.1.hex} jupyter_domain=${var.jupyter_domain} binder_domain=${var.binder_domain} TLS_email=${var.TLS_email} cpu_alloc=${var.cpu_alloc} mem_alloc=${var.mem_alloc_gb} bash /tmp/install-binderhub.sh"
    ]
  }
}
